...
# Docker Hub 镜像名
DOCKERHUB_IMAGE="${DOCKERHUB_USERNAME}/${CNB_REPO_NAME_LOWERCASE}:${TAG}"
...
cd "$PROJECT_ROOT"
docker build -t "$CNB_REPO_NAME_LOWERCASE:$TAG" .
...
docker tag "$CNB_REPO_NAME_LOWERCASE:$TAG" "$DOCKERHUB_IMAGE"
...
docker tag "$CNB_REPO_NAME_LOWERCASE:$TAG" "$CNB_IMAGE"
